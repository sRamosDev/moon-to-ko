...
class MigrationApp(ctk.CTk):
    def __init__(self):
        ...
        self.title_label = ctk.CTkLabel(
            self,
            text="Moon+ to KOReader Migrator",
        ...
        self.options_frame.grid(
            row=3, column=0, columnspan=3, padx=20, pady=15, sticky="ew"
        )
        self.options_frame.grid_columnconfigure(0, weight=1)
        self.options_frame.grid_columnconfigure(1, weight=1)

        self.extract_epubs_var = ctk.BooleanVar(value=True)
        self.epubs_chk = ctk.CTkCheckBox(
            self.options_frame, text="Extract EPUBs", variable=self.extract_epubs_var
        )
        self.epubs_chk.grid(row=0, column=0, padx=20, pady=15)

        self.extract_replacements_var = ctk.BooleanVar(value=True)
        self.replacements_chk = ctk.CTkCheckBox(
            self.options_frame,
            text="Extract Text Replacements",
            variable=self.extract_replacements_var,
        )
        ...
        self.status_label = ctk.CTkLabel(
            self, textvariable=self.status_var, font=ctk.CTkFont(size=13)
        )
        self.status_label.grid(row=4, column=0, columnspan=3, padx=20, pady=(5, 5))

        self.progressbar = ctk.CTkProgressBar(self)
        self.progressbar.grid(
            row=5, column=0, columnspan=3, padx=20, pady=(0, 15), sticky="ew"
        )
    ...
    def browse_input(self):
        filename = filedialog.askopenfilename(
            title="Select Moon+ Backup", filetypes=[("Moon+ Backup Files", "*.mrpro")]
        )
        if filename:
    ...
    def start_migration(self):
        ...
        if not in_path or not out_path:
            self.status_var.set(
                "Error: Please select both an input file and output directory."
            )
            return
        ...
        thread = threading.Thread(
            ...
            args=(
                in_path,
                out_path,
                self.extract_epubs_var.get(),
                self.extract_replacements_var.get(),
            ),
        )
        thread.start()
        ...
        def update_progress(current, total):
            if total > 0:
                ...
                self.after(
                    0,
                    self.status_var.set,
                    f"Extracting EPUBs: {current}/{total} ({(prog * 100):.1f}%)",
                )
